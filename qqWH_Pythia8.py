...
for i in range(n):
    ...
    ############# WH HAD-HAD SELECTION ####################
    Lepton_list = []
    if not totTaus == 2: continue
    if not totLeps == 1: continue

    if totElecs == 1: Lepton_list = elecs
    if totMuons == 1: Lepton_list = muons 

    # 26 GeV lepton to ensure trigger
    good_pt = False
    pT = tt.mc_pt[Lepton_list[0]]
